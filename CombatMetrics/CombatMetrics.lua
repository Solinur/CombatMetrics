...
local StatDebuffs = {
	...
	}, -- Crusher, Target Dummy (the following line might overwrite this. If LUI extended is used, both declarations are necessary)
	[GetFormattedAbilityName(17906)] = {
		[LIBCOMBAT_STAT_SPELLPENETRATION] = 2108,
		[LIBCOMBAT_STAT_WEAPONPENETRATION] = 2108,
	}, -- Crusher, can get changed by settings !
	...
	}, -- Alkosh, Target Dummy (the following line might overwrite this. If LUI extended is used, both declarations are necessary)
	...
	[GetFormattedAbilityName(178118)] = {
		[LIBCOMBAT_STAT_SPELLPENETRATION] = WrathOfNaturePen,
		[LIBCOMBAT_STAT_WEAPONPENETRATION] = WrathOfNaturePen,
	}, -- Overcharged
...
function CMX.SetPenetrationDebuffValue(debuffKey, value)
	...
	if value == nil then
		...
		db[debuffKey] = value
	end -- load values from SV when no value is given, otherwise store new value
...
function UnitHandler:UpdateStats(fight, effectdata, abilityId)
	...
	for stat, value in pairs(debuffStatData) do
		...
		if StatusEffectIds[abilityId] and fight.special["wrathCP"] ~= true then
			return
		end
		...
		for tablekey, list in pairs(CategoryList) do -- iterate over categories, i.e. damageOut, list is a category specific list of tablekeys, which each of the abilities h
			for _, ability in pairs(unit[tablekey]) do -- iterate over abilities
				...
				if tablekey == "damageOut" then
					ability.damageOutTotal = ability.damageOutNormal
						+ ability.damageOutCritical
						+ ability.damageOutBlocked
	...
	for unitId, _ in pairs(unitselection or data.units) do -- if a selection was made the content of the value will be "true" and not the table from the original data.
		...
		if isNotEmpty and (isEnemy == isDamageCategory) and unitData then
			for name, buff in pairs(unit.buffs) do
				local selectedbuff = selectiondata.buffs[name]
					or { uptime = 0, count = 0, groupUptime = 0, groupCount = 0, maxStacks = 0 }
...
local function ProcessLogDamage(fight, logline)
	local callbacktype, timems, result, sourceUnitId, targetUnitId, abilityId, hitValue, damageType, overflow =
		unpackLogline(logline, 1, 9)

	if timems < (fight.combatstart - 500) or fight.units[sourceUnitId] == nil or fight.units[targetUnitId] == nil then
		return
	end
	...
	if callbacktype == LIBCOMBAT_EVENT_DAMAGE_OUT then
		...
		abilitydata =
			fight:AcquireUnitData(sourceUnitId, timems):AcquireAbilityData(abilityId, ispet, damageType, "damageIn")
		isDamageOut = false
...
local function ProcessLogHeal(fight, logline, overrideCallbackType)
	local callbacktype, timems, result, sourceUnitId, targetUnitId, abilityId, hitValue, powerType, overflow =
		unpackLogline(logline, 1, 9)
	...
	if callbacktype == LIBCOMBAT_EVENT_HEAL_OUT then
		abilitydata =
			fight:AcquireUnitData(targetUnitId, timems):AcquireAbilityData(abilityId, ispet, powerType, "healingOut") -- get table for ability (within the unittable)
		isHealingOut = true
		...
		abilitydata =
			fight:AcquireUnitData(sourceUnitId, timems):AcquireAbilityData(abilityId, ispet, powerType, "healingIn")
		isHealingOut = false
	...
	if hitValue == 0 and overflow > 0 then
		abilitydata[overflowHealskey] = abilitydata[overflowHealskey] + 1
	end
...
local function ProcessLogEffects(fight, logline)
	local callbacktype, timems, unitId, abilityId, changeType, effectType, stacks, sourceType, slotId, hitValue =
		unpackLogline(logline, 1, 10)

	local currentstacks = stacks or 0

	if timems < (fight.combatstart - 500) or fight.units[unitId] == nil then
		return
	end
...
local function ProcessLogSkillTimings(fight, logline)
	...
	elseif status == LIBCOMBAT_SKILLSTATUS_BEGIN_DURATION or status == LIBCOMBAT_SKILLSTATUS_BEGIN_CHANNEL then
		...
		if lastRegisteredIndex == nil then
			...
			castData[lastRegisteredIndex][4] = timems
			castData[lastRegisteredIndex][5] = timems
				+ CMX_GetAbilityDuration(abilityId)
				+ (abilityDelay[abilityId] or 0)
			table.insert(skill.times, timems)
...
local function ProcessMessages(fight, logline)
	...
	if messageId ~= LIBCOMBAT_MESSAGE_WEAPONSWAP then
		return
	end
...
local function ProcessPerformanceStats(fight, logline)
	...
	if not (avg and min and max and ping) then
		return
	end
...
local function FinalizeUnitBuffs(fight)
	...
	for unitId, unitData in pairs(fight.units) do
		local unitCalc = calc.units[unitId] -- calculated info is not stored in fight.units but in fight.calculated.units
...
local function FinalizeResourceStats(fight)
	local resources = fight.calculated.resources
...
local function FinalizeStats(fight)
	local calcData = fight.calculated
	local stats = calcData.stats

	-- calculate damage sums for the relevant categories

	local damageOut = calcData.damageOut

	calcData.damageOutSpells = {}
	initBaseAbility(calcData.damageOutSpells, "damageOut")

	calcData.damageOutWeapon = {}
	initBaseAbility(calcData.damageOutWeapon, "damageOut")

	for id, ability in pairs(damageOut) do
		local isMagic = IsMagickaAbility[ability.damageType]
		local datatable = (isMagic == true and calcData.damageOutSpells)
			or (isMagic == false and calcData.damageOutWeapon)
			or {}
	...
	if calcData.damageOutSpells.min == inf then
		calcData.damageOutSpells.min = 0
	end
	if calcData.damageOutWeapon.min == inf then
		calcData.damageOutWeapon.min = 0
	end

	local damageIn = calcData.damageIn

	calcData.damageInSpells = 0
	calcData.damageInWeapon = 0

	for id, ability in pairs(damageIn) do
		local isMagic = IsMagickaAbility[ability.damageType]

		if isMagic == true then
			calcData.damageInSpells = calcData.damageInSpells + ability.damageInTotal
		elseif isMagic == false then
			calcData.damageInWeapon = calcData.damageInWeapon + ability.damageInTotal
		end
	end
		...
		for statId, stattype in pairs(list) do
			...
			if statdata ~= nil then
				...
				if statId == LIBCOMBAT_STAT_STATUS_EFFECT_CHANCE then
					totaldmgvalue = zo_max(calcData.damageOutTotal, 1)
				end
				...
				if statdata.healsum ~= nil and stattype ~= STATTYPE_PENETRATION then
					healValue = statdata.healsum / totalhealvalue
				end
				if statdata.min == inf then
					statdata.min = 0
				end
...
local function FinalizeSkillTimings(fight)
	local calcData = fight.calculated
	local skillData = calcData.skills
	local castData = calcData.casts
	...
	for i = #castData, 1, -1 do -- go backwards to allow deleting without messing up indices
		...
		if startTime and not ignoredAbilityTiming[skillId] then
			...
			endTime = isWeaponAttack == false and zo_max(startTime + 1000, endTime or 0)
				or (endTime or startTime + 1000)
			...
			if isWeaponAttack then
				if lastValidWeaponAttack and lastValidWeaponAttack - i == 1 then
					skill.weavingErrors = skill.weavingErrors + 1
				end
				lastValidWeaponAttack = i
			else
				if lastValidSkill and lastValidSkill - i == 1 then
					skill.weavingErrors = skill.weavingErrors + 1
				end
				lastValidSkill = i
			end
	...
	for reducedslot, skill in pairs(skillData) do
		...
		if skillId then
			...
			if delayCount and delayCount > 0 then
				skill.delayAvg = skill.delaySum / delayCount
			end
			if weavingTimeCount and weavingTimeCount > 0 then
			...
			if count > 1 then
				skill.diffTimeAvg = (timedata[#timedata] - timedata[1]) / (count - 1)
			end
	...
	calcData.delayAvg = (totalDelayCount > 0 and totalDelay / totalDelayCount) or 0

	calcData.casts = nil
	calcData.lastIndex = nil
end

local function FinalizeBarData(fight)
	local barData = fight.calculated.barStats
...
local function FinalizePerformanceData(fight)
	local performance = fight.calculated.performance
...
local function Finalize(fight)
	...
	fight:FinalizeUnitBuffs()
	fight:AccumulateStats()
	fight:FinalizeResourceStats()
	fight:FinalizeStats()
	fight:FinalizeSkillTimings()
	fight:FinalizeBarData()
	fight:FinalizePerformanceData()

	-- set player buffs shortcut
...
local function CalculateChunk(fight) -- called by CalculateFight or itself
	...
	for i = istart + 1, iend do
		...
		if ProcessLog[logType] then
			ProcessLog[logType](fight, logline)
		end
	end
	...
	if iend >= #logdata then
		...
		fight.cindex = iend
		em:RegisterForUpdate("CMX_chunk", 20, function()
			fight:CalculateChunk()
		end)
	end
	...
	local newchunksize =
		zo_min(zo_ceil(desiredtime / zo_max(chunktime, 0.001) * db.chunksize / stepsize) * stepsize, 20000)
...
local function AddtoChatLog(logType, ...)
	...
	local isEnabled = (
		(logType == LIBCOMBAT_EVENT_DAMAGE_OUT or logType == LIBCOMBAT_EVENT_DAMAGE_SELF)
		and db.chatLog.damageOut == true
	)
	...
	if isEnabled then
		...
		if chatContainer and text and color then
			chatContainer:AddMessageToWindow(chatWindow, text, unpack(color))
		end -- in case the container got removed by the user or an addon
	end
end

local function AddToLog(logType, ...)
	if LC.data.inCombat ~= true and logType == LIBCOMBAT_EVENT_PERFORMANCE then
		return
	end
	table.insert(CMX.currentdata.log, { logType, ... })
	if db.chatLog.enabled then
		AddtoChatLog(logType, ...)
	end
end
	...
	if #lastfights > db.fighthistory then
		...
		if db.keepbossfights then
			for i = 1, #lastfights - 1 do
				if not lastfights[i].bossfight then
					fighttodelete = i
					break
				end
			end
...
local function GetFightName(fight)
	...
	for k, unitData in pairs(fight.units) do
		if fight.bossfight == true and unitData.bossId ~= nil and unitData.damageOutTotal > dmgmax then -- find the "biggest" enemy
			...
			if unitData.bossId == 1 then
				break
			end -- this should be the name of the main boss
...
local function AddFightCalculationFunctions(fight)
	fight.GetFightName = GetFightName
	...
	fight.FinalizeSkillTimings = FinalizeSkillTimings
	fight.FinalizeBarData = FinalizeBarData
	fight.FinalizePerformanceData = FinalizePerformanceData
end
...
local function FightSummaryCallback(_, fight)
	...
	if SCENE_MANAGER.currentScene.name == "CMX_REPORT_SCENE" then
...
local function UpdateEvents(event)
	...
	local loadgroupevents = isGrouped
...
local function maxStat()
	...
	if stamina > magicka then
		maxPower = POWERTYPE_STAMINA
	end
	if health > magicka and health > stamina then
		maxPower = POWERTYPE_HEALTH
	end
...
local function Initialize(event, addon)
	...
	if svtable then
		for k, v in pairs(svtable) do
			if v.version and v.version < 5 then
				svtable[k] = nil
			end
		end
	end
	...
	if not CMX.db.accountwide then
		CMX.db = ZO_SavedVars:NewCharacterIdSettings("CombatMetrics_Save", 5, "Settings", svdefaults)
	end
	...
	if db.chatLog.enabled then
		zo_callLater(CMX.InitializeChat, 500)
	end
...
em:RegisterForEvent(CMX.name, EVENT_ADD_ON_LOADED, function(...)
	Initialize(...)
end)
