...
SafeAddString(
	SI_COMBAT_METRICS_EDIT_TITLE,
	"Двойной щелчок, чтобы отредактировать название битвы",
	1
)

SafeAddString(SI_COMBAT_METRICS_DAMAGE_DONE, "Урон Нанесённый", 1)
...
SafeAddString(
	SI_COMBAT_METRICS_POSTALLDPS,
	"Вывести в чат общий DPS и по одиночной цели",
	1
)
...
SafeAddString(
	...
	"Файл сохранений переполнен. Удалите битву, чтобы освободить немного места!",
	1
)
...
SafeAddString(
	SI_COMBAT_METRICS_SAVE_FIGHT2,
	"Shift+Щелчок: Сохранить битву с логом боя",
	1
)
...
SafeAddString(
	SI_COMBAT_METRICS_TOGGLE_GROUPBUFFIN_EVENTS,
	"Входящие события групповых баффов",
	1
)
SafeAddString(
	SI_COMBAT_METRICS_TOGGLE_GROUPBUFFOUT_EVENTS,
	"Исходящие события групповых баффов",
	1
)
SafeAddString(SI_COMBAT_METRICS_TOGGLE_RESOURCE_EVENTS, "События ресурсов", 1)
SafeAddString(
	SI_COMBAT_METRICS_TOGGLE_STATS_CHANGE_EVENTS,
	"События изменения характеристик",
	1
)
SafeAddString(
	SI_COMBAT_METRICS_TOGGLE_MESSAGE_CHANGE_EVENTS,
	"Информационные события, например, переключение оружия",
	1
)
...
SafeAddString(
	...
	"Настоящий DPS, т.е. урон в секунду между вашим первым и последним ударом по этой цели",
	1
)
...
SafeAddString(
	...
	"Если включено, текущие настройки будут применены ко всем персонажам на аккаунте",
	1
)
...
SafeAddString(
	...
	"Используйте с осторожностью! Большое количество сохранённых данных значительно увеличивает время загрузки. Если файл станет слишком большим, клиент может упасть при попытке открыть его.",
	1
)
...
SafeAddString(
	...
	"Сначала удаляются битвы с трэшем, а затем уже битвы с боссами, при достижении лимита сохраняемых битв",
	1
)
...
SafeAddString(
	SI_COMBAT_METRICS_MENU_STACKS_TOOLTIP,
	"Показывает обособленные стаки на панели баффов",
	1
)

SafeAddString(SI_COMBAT_METRICS_MENU_GL_NAME, "Урон в большой группе", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_GL_TOOLTIP,
	"Отслеживать урон всей группы в больших группах (более 4 человек в группе)",
	1
)
...
SafeAddString(
	...
	"В облегчённом режиме Combat Metrics считает только текущий DPS/HPS в режиме реального времени. Статистика не подсчитывается и не записывается, большое окно отчёта отключено",
	1
)
...
SafeAddString(
	...
	"Выключает логирование битвы в Сиродиле",
	1
)
...
SafeAddString(
	...
	"Переключение на облегчённый режим в Сиродиле. В облегчённом режиме Combat Metrics считает только текущий DPS/HPS в режиме реального времени. Статистика не подсчитывается и не записывается, большое окно отчёта отключено",
	1
)
...
SafeAddString(
	...
	"Автоматический выбор канала чата при выводе отчёта о DPS/HPS. В группе канал чата /group имеет приоритет перед каналом чата /say.",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_AS_NAME, "Автоскриншот", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_AS_TOOLTIP,
	"Автоматическое создание скриншота при открытии окна отчёта",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_ML_NAME, "Длительность битвы для скриншота", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_ML_TOOLTIP,
	"Минимальная длительность битвы в секундах для создания автоматического скриншота",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_SF_NAME, "Размер окна отчёта о битве", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_SF_TOOLTIP,
	"Задаёт масштаб всех элементов окна отчёта о битве",
	1
)
...
SafeAddString(
	...
	"Снижение сопротивляемости от баффа с глифа Crusher. Для золотого глифа максимального уровня: обычный: 1622, infused: 2108",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_MOBRESISTANCE, "Сопротивляемость цели", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_MOBRESISTANCE_TOOLTIP,
	"Сопротивляемость цели, используемая для расчёта сверхпробивания",
	1
)
...
SafeAddString(
	...
	"Включает окно текущей статистики, которое показывает DPS & HPS во время боя",
	1
)
...
SafeAddString(
	...
	"Блокирует окно текущей статистики, его будет нельзя переместить",
	1
)
...
SafeAddString(
	...
	"Выравнивает цифры урона/исцеления и т.п. в окне текущей статистики по левому краю",
	1
)
...
SafeAddString(
	...
	"Выберите формат окна текущей статистики",
	1
)
...
SafeAddString(
	...
	"Транслировать события Урона и Исцеления в окно чата",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_CHAT_TITLE_NAME, "Лог в чат", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_CHAT_TITLE_TOOLTIP,
	"Показывает в чате в режиме реального времени наносимый вами урон",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_CHAT_SD_NAME, "Урон", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_CHAT_SD_TOOLTIP,
	"Показывает наносимый вами Урон в окно чата",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_CHAT_SH_NAME, "Исцеление", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_CHAT_SH_TOOLTIP,
	"Показывает наносимое вами Исцеление в окно чата",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_CHAT_SID_NAME, "Входящий Урон", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_CHAT_SID_TOOLTIP,
	"Показывает Входящий Урон в окно чата",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_CHAT_SIH_NAME, "Входящее Исцеление", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_CHAT_SIH_TOOLTIP,
	"Показывает Входящее Исцеление в окно чата",
	1
)
...
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_SF_TOOLTIP,
	"Выводит результаты боя в системное окно чата",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_DEBUG_SA_NAME, "ID способностей", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_SA_TOOLTIP,
	"Показывает ID способностей в окне отчёта",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_DEBUG_SFC_NAME, "Инфо подсчёта битвы", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_SFC_TOOLTIP,
	"Выводит информацию о подсчёте таймингов в системное окно чата",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_DEBUG_BI_NAME, "Инфо Баффов", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_BI_TOOLTIP,
	"Выводит события Баффов в системное окно чата (Заспамливание)",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_DEBUG_US_NAME, "Использованные способности", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_US_TOOLTIP,
	"Выводит события использованных способностей в системное окно чата",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_DEBUG_SG_NAME, "Инфо о группе", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_SG_TOOLTIP,
	"Выводит информацию о присоединении и покидании группы в системное окно чата",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_DEBUG_MD_NAME, "Прочая отладочная инфа", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_MD_TOOLTIP,
	"Выводит информацию о прочих событиях боя в системное окно чата",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_DEBUG_SPECIAL_NAME, "Особая отладочная инфа", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_SPECIAL_TOOLTIP,
	"Выводит информацию об особых событиях боя в системное окно чата",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_DEBUG_SAVE_NAME, "Инфо о сохр. данных", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_SAVE_TOOLTIP,
	"Выводит отладочную информацию о сохранённых и загруженных битвах в системное окно чата",
	1
)
