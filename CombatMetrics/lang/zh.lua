...
SafeAddString(SI_COMBAT_METRICS_DAMAGE_DONE, "造成伤害", 1)
...
SafeAddString(
	SI_COMBAT_METRICS_POSTBUFF_FORMAT,
	"<<1>> - 持续时间: <<2>> (<<3>><<4[/ 于 $d/ 于 $d 持续时间]>>)",
	1
) -- (<<1>> = buff name, <<2>> = relative uptime, <<3>> = uptime, <<4>> = time) e.g. Major Intellect - Uptime: 93.2% (9:26 in 10:07)
SafeAddString(
	SI_COMBAT_METRICS_POSTBUFF_FORMAT_GROUP,
	"<<1>> - 持续时间: <<2>>/<<5>> (<<3>>/<<6>><<4[/ 于 $d/ 于 $d 持续时间]>>)",
	1
...
SafeAddString(
	...
	"如果你想从皇冠商城送礼，我很乐意收到一些皇冠宝箱或消耗品。\n如果你想送什么其他的东西，你也可以联系我。",
	1
)
...
SafeAddString(
	...
	"存储文件已满。此战斗储存需要 <<1>> MB 空间。删除一个战斗以释放一些空间，或者在设置中增加允许使用的容量空间!",
	1
)
...
SafeAddString(
	...
	"真实DPS, 例如对该目标造成的第一击和最后一击之间的每秒伤害值",
	1
)
...
SafeAddString(
	...
	"挥舞时间\n\n从本次到下一次技能释放中间浪费的平均时间。",
	1
)
SafeAddString(
	SI_COMBAT_METRICS_SKILLTIME_TT3,
	"挥舞错误\n\n技能激活后没有立即进行武器攻击的次数，反之亦然。",
	1
)
...
SafeAddString(
	...
	"使用注意! 大量的保存数据会大大增加读取时间。如果文件变得过大, 客户端可能会在试图读取数据时崩溃。",
	1
)
...
SafeAddString(
	...
	"当战斗总数限制达到时，先删除垃圾战斗数据，之后才删除boss战斗数据",
	1
)
...
SafeAddString(
	...
	"在轻量模式中, 实时报告窗口中战斗度量将仅计算 DPS/HPS 。将不会计算统计数据并且战斗报告窗口将被禁用",
	1
)
...
SafeAddString(
	...
	"在悉罗帝尔地图和角斗场切换为轻量模式。在轻量模式中, 实时报告窗口中战斗度量将仅计算 DPS/HPS 。将不会计算统计数据并且战斗报告窗口将被禁用",
	1
)
...
SafeAddString(
	...
	"在聊天窗口中贴出 DPS/HPS 时自动选择频道。在队伍中时使用 /group 聊天频道，否则使用 /say 聊天频道。",
	1
)
...
SafeAddString(
	...
	"有时，我可能会在报告窗口添加一个通知，例如收集数据或招募人员到我的队伍里(为了节省时间，我宁愿使用插件)。如果你不想要这个，就把它关掉。",
	1
)
...
SafeAddString(
	...
	"由粉碎者附魔debuff造成的抗性降低。以最大等级金附魔: 标准: 1622, 魔甲强化: 2108, 魔甲强化 + 塔鲁格契约: 2740",
	1
)
...
SafeAddString(
	...
	"使用请注意! 创建文本杭需要占用大量CPU。当你需要大量的战斗时最好禁用此选项 (试炼, 悉罗帝尔)",
	1
)
