...
SafeAddString(SI_COMBAT_METRICS_DAMAGE_DONE, "Dano Causado", 1)
...
SafeAddString(
	...
	"<<1>> - Atividade: <<2>>/<<5>> (<<3>>/<<6>><<4[/ on $d/ on $d units]>>)",
	1
...
SafeAddString(
	...
	"\nSe envontrar um bug, tem uma requisição ou uma sugestão, envie um email no jogo, crie uma pergunta no GitHub ou poste nos comentários no EsoUI. \n\nDoações são apreciadas mas não obrigatórias ou necessárias. \nSe você quer doar dinheiro real por favor visite o site do addon no EsoUI",
	1
)

SafeAddString(
	SI_COMBAT_METRICS_STORAGE_FULL,
	"O arquivo de armazenamento está cheio. A luta que você quer salvar precisa de <<1>> MB e <<2>> indices. Apque uma luta para liberar algum espaço ou aumente o espaço permitido nas configurações!",
	1
)
...
SafeAddString(
	...
	"DPS Real, ex. o dano por segundo entre seu primeiro e seu ultimo golpe no alvo",
	1
)
...
SafeAddString(
	...
	"Tempo entre ativação da habilidade e a próxima ativação de arma/Habilidade.",
	1
)
...
SafeAddString(
	...
	"Se habilitado todos os personagens de uma conta vão compartilhar suas configurações",
	1
)
...
SafeAddString(
	...
	"Use com cuidado! Muitos dados salvos aumentam significativamente o tempo de carregamento. Se o arquivo ficar muito grande, o cliente pode dar erro quando tentar ler.",
	1
)
...
SafeAddString(
	...
	"Apaga as lutas lixo primeiro, antes de apagar as lutas com os chefes quando o limite de lutas for alcançado",
	1
)
...
SafeAddString(
	...
	"Quando no modo leve, o combat metrics só vai calcular o DPS/CPS na janela de relatório ativa. Nenhuma estatística será calculada e a janela grande de relatório será desativada",
	1
)
...
SafeAddString(
	...
	"Alterna para o modo leve em Cyrodil. Quando no modo leve, o Combat Metrics só calcula o DPS/CPS na janela de relatório ativa. Nenhuma estatística será calculada e a janela grande de relatório será desativada",
	1
)
...
SafeAddString(
	...
	"Automaticamente escolhe o canal quando postar o DPS/CPS no chat. Quando em grupo o chat /group é usado senão usa o chat /say.",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_AS_NAME, "Auto Screenshot", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_AS_TOOLTIP,
	"Automaticamente tira uma Screenshot quando abrir a janela de Relatório",
	1
)
...
SafeAddString(
	...
	"Ajusta o tamanho de todos os elementos da janela de Relatório de Luta",
	1
)
...
SafeAddString(
	...
	"Redução de resistência devido à penalidade do glifo Esmagador. Para o glifo dourado nível máximo: padrão: 1622, infundido: 2108",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_MOBRESISTANCE, "Resistência do Alvo", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_MOBRESISTANCE_TOOLTIP,
	"Resistência do Alvo que é assumida para o cálculo de overpenetração",
	1
)
...
SafeAddString(
	...
	"Habilita a Janela de Relatório Ativa que mostra DPS & CPS durante o combate",
	1
)
...
SafeAddString(
	...
	"Travar a Janela de relatórios ao ativa, para ela não se mover",
	1
)
...
SafeAddString(
	...
	"Alinhe a esquerda os conjuntos de números de Dano/Curas/etc. para a hanela de relatórios ao ativa",
	1
)
...
SafeAddString(
	...
	"Envia os dados dos Eventos de Dano e Cura para a janela de chat",
	1
)
...
SafeAddString(
	...
	"Mostra os ids das habilidades na janela de relatório de luta",
	1
)
...
SafeAddString(
	...
	"Imprime as Informações sobre o cálculo dos tempos na janela de chat do sistema",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_DEBUG_BI_NAME, "Mostra Info de Bônus", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_BI_TOOLTIP,
	"Imprime os events de Bônus na janela de chat do sistema (Spammy)",
	1
)
...
SafeAddString(
	...
	"Imprime os eventos de habilidades usadas na janela de chat do sistema",
	1
)
SafeAddString(SI_COMBAT_METRICS_MENU_DEBUG_SG_NAME, "Mostra Info do Grupo", 1)
SafeAddString(
	SI_COMBAT_METRICS_MENU_DEBUG_SG_TOOLTIP,
	"Imprime eventos de entrar e sair do grupo na janela de chat do sistema",
	1
)
...
SafeAddString(
	...
	"Imprime certos eventos especiais na janela de chat do sistema",
	1
)
...
SafeAddString(
	...
	"Imprime informações de debug sobre lutas salvas e lidas na janela de chat do sistema",
	1
)
