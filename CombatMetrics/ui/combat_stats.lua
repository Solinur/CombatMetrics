...
function CombatStatsPanel:Update(fightData)
	...
	local header2 = self:GetNamedChild("StatHeaderLabel2")
	...
	local dpsRow = self:GetNamedChild("StatRowAPS")

	dpsRow:GetNamedChild("Label"):SetText(label1)                             -- DPS or HPS
	self:GetNamedChild("StatTitleAmount"):GetNamedChild("Label"):SetText(label2) -- Damage or Healing
	self:GetNamedChild("StatTitleCount"):GetNamedChild("Label"):SetText(label3) -- Hits or Heals
	...
	self:GetNamedChild("ActiveTimeValue"):SetText(activetimestring)
	self:GetNamedChild("CombatTimeValue"):SetText(combattimestring)
	...
	for k, v in ipairs(rowList) do
		local rowcontrol1 = self:GetNamedChild("StatRowAmount" .. k)
		local rowcontrol2 = self:GetNamedChild("StatRowCount" .. k)
